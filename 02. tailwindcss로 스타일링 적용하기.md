# 1. Next.js 스타일링 시 tailwind를 권장하는 이유?
<details>
  <summary>1-1. 빠르고 유연한 스타일링</summary>

  * **`utility-first CSS framework`**: 먼저 유틸리티 클래스를 써보고, 정말 필요할 때만 커스텀 CSS를 작성하자
    → 사전에 정의되어 있는 `유틸리티 클래스`를 조합해서 빠르고 유연하게 스타일링 진행 가능
...
<details>
  <summary>1-2. Zero-Runtime 스타일시트</summary>

  * `tailwind`와 자주 비교되는 `styled-components`(또는 `emotion`)의 경우, `런타임`에 스타일을 생성함
    * 즉, 사용자가 사이트를 사용할 때, 자바스크립트가 실시간으로 스타일을 생성하고 DOM에 삽입함
  * `tailwind`는 별도의 설정없이도 **`빌드 단계에서 스타일시트 미리 생성`**이 가능함
  ```JavaScript
  <button class="text-red-500 bg-white px-4 py-2">Click</button>
  ```
  * 브라우저 실행 중에 자바스크립트가 스타일을 만들지 않음
  * `tailwind`는 실행 중에 스타일을 만들지 않고, **빌드할 때 미리 필요한 CSS만 추려서 정적으로 만들어 놓는 방식**이기 때문에 `zero-runtime` 스타일시트라고 부름
</details>

<details>
  <summary>1-3. tailwind 에서의 reset css 및 커스텀 기능(자주 사용 되는 기능 모음)</summary>

  Next.js에서 tailwind로 보일러 플레이트를 만들면 별도의 reset css 설정 없이 바로 사용 가능함
  **⬇️ globals.css**
  ```CSS
  @import "tailwindcss";

  /* CDN 방식으로 웹폰트 다운받고자 할 때 */
  @importurl("https://cdn.jsdelivr.net/gh/orioncactus/pretendard@v1.3.9/dist/web/variable/pretendardvariable.min.css");

/* 다크 모드 사용 설정: 스타일 적용 시 className="bg-white dark:bg-black" */
  @custom-variant dark (&:where(.dark, .dark *));

  @theme inline {
    --color-background: var(--background); /* className="text-background" */
    --color-foreground: var(--foreground);
    --font-sans: var(--font-geist-sans);
    --font-mono: var(--font-geist-mono);  /* className="font-mono"로 적용 */
  }

  /* ===== 테마 정의 ===== */
  @theme {
    /* 브레이크포인트 정의: 기본 반응형 브레이크 포인트를 디자인 요구사항에 따라 변경 가능 */
    /* className="text-sm md:text-2xl"면 화면너비 기본 text-sm, 786px부터 text-2xl 의미 */
    --breakpoint-sm: 640px;
    --breakpoint-md: 768px;
  ...
  /* 커스텀 유틸리티 클래스 정의. 실사용 시 className="tab-4" */
  @utility tab-4 {
    tab-size: 4;
  }
  ...
  /* 카드 컴포넌트 */
  .card {
    @apply card-base shadow-card p6;
  }

  /* 일반 HTML 태그에 대한 기본 스타일. 가능하지만 JIT 최적화 안되니 가급적 유틸리티 클래스 사용할 것 */
  body {
...
<details>
  <summary>1-4. VS CODE Extension 에서 Tailwind CSS IntelliSence 설치하기</summary>
</details>

___

# 2. tailwind 주요 클래스 설명
```JavaScript
